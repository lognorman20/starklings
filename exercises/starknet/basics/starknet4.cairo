...
trait ILizInventory<TContractState> {
    fn add_stock(ref self: TContractState, product: felt252, incoming_stock: u32);
...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, incoming_stock: u32) {
            // TODO:
            // * takes product and new_stock
            // * adds new_stock to stock in inventory
            // * only owner can call this
            assert(self.contract_owner.read() == get_caller_address(), 'get ya money up broke boy');
            let new_stock = self.inventory.read(product) + incoming_stock;
            self.inventory.write(product, new_stock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let new_quantity = self.inventory.read(product) - quantity;
            self.inventory.write(product, new_quantity);
        }
